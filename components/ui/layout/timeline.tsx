...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  useEffect(() => {
    if (ref.current) {
      ...
      // Calculate the actual content height more accurately for mobile
      const isMobile = window.innerWidth < 768;

      if (isMobile) {
        // On mobile, items have pt-10 (40px) spacing vs md:pt-40 (160px) on desktop
        // Calculate based on actual rendered content
        const items = ref.current.querySelectorAll(".flex.justify-start");
        if (items.length > 0) {
          const lastItem = items[items.length - 1] as HTMLElement;
          const lastItemRect = lastItem.getBoundingClientRect();
          const containerRect = ref.current.getBoundingClientRect();
          // Calculate height from top of container to about halfway through the last item
          // Use at least 160px (top-40 sticky position) but extend to cover content
          const calculatedHeight =
            lastItemRect.top -
            containerRect.top +
            Math.max(200, lastItemRect.height * 0.5);
        ...
        if (isMobile) {
          ...
          if (items.length > 0) {
            ...
            const calculatedHeight =
              lastItemRect.top -
              containerRect.top +
              Math.max(160, lastItemRect.height * 0.5);
            setHeight(calculatedHeight);
          } else {
            setHeight(rect.height * 0.6);
          }
        } else {
          setHeight(rect.height);
        }
